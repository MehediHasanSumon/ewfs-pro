...
<head>
    <meta charset="utf-8">
    <title>White Sale Invoice</title>
    <style>
        body {
...
<body>
    ...
    <div class="title-section">
        <div class="title-box">Bill Details</div>
    </div>
    ...
    <div class="invoice-info">
        <table>
            <tr>
                <td class="left">
                    <strong>Company:</strong> {{ $whiteSale->company_name }}<br>
                    <strong>Proprietor:</strong> {{ $whiteSale->proprietor_name }}<br>
                    <strong>Mobile:</strong> {{ $whiteSale->mobile_no }}<br>
                </td>
                <td class="right">
                    <strong>Invoice No:</strong> {{ $whiteSale->invoice_no }}<br>
                    <strong>Date:</strong> {{ \Carbon\Carbon::parse($whiteSale->sale_date)->format('d/m/Y') }}<br>
                    <strong>Time:</strong> {{ $whiteSale->sale_time }}<br>
                    <strong>Shift:</strong> {{ $whiteSale->shift->name ?? 'N/A' }}<br>
                </td>
            </tr>
        </table>
    </div>
    ...
    <table>
        ...
        <tbody>
            @foreach ($whiteSale->products as $index => $product)
                <tr>
                    ...
                    <td class="text-right">{{ number_format($product->sales_price, 2) }}</td>
    ...
    <div class="total-section">
        <p class="amount">Total Amount: {{ number_format($whiteSale->total_amount, 2) }}</p>
        <p class="words">In words: {{ numberToWords(floor($whiteSale->total_amount)) }}</p>
        @if ($whiteSale->remarks)
            <p><strong>Remarks:</strong> {{ $whiteSale->remarks }}</p>
        @endif
    </div>
    ...
    @php
        function numberToWords($num)
        {
            ...
            function convertLessThanThousand($n, $ones, $tens, $teens)
            {
                ...
                return $ones[floor($n / 100)] .
                    ' Hundred' .
                    ($n % 100 != 0 ? ' ' . convertLessThanThousand($n % 100, $ones, $tens, $teens) : '');
            }
            ...
            $result = '';
            if ($billion > 0) {
                $result .= convertLessThanThousand($billion, $ones, $tens, $teens) . ' Billion ';
            }
            if ($million > 0) {
                $result .= convertLessThanThousand($million, $ones, $tens, $teens) . ' Million ';
            }
            if ($thousand > 0) {
                $result .= convertLessThanThousand($thousand, $ones, $tens, $teens) . ' Thousand ';
            }
            if ($remainder > 0) {
                $result .= convertLessThanThousand($remainder, $ones, $tens, $teens);
            }
